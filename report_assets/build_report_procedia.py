"""Converts the research paper into the Procedia-style conference template the user
supplied (report_assets/_template_readonly.docx -- a content-type-patched, read-only
copy of their .docm; no macros are executed or preserved). NOT a rewrite: every
sentence of body text below is copied verbatim from build_report.py (the source the
user pointed at), just re-applied with the template's own named styles
(Els-Title, Els-1storder-head, Els-body-text, Els-caption, Els-reference, etc.)
instead of generic formatting.

Run: python report_assets/build_report_procedia.py
"""
import os
...
TEMPLATE = os.path.join(HERE, "_template_readonly.docx")
OUT = os.path.join(HERE, "Nikunj_Bhalla_2430030052_ETE_Research_Paper_Procedia.docx")
FIG = HERE
...
# ---- truncate the template's placeholder/instructional content -----------
# Keep paragraphs 0-7 (DocHead, Els-Title, Els-Author, Els-Affiliation x2,
# Els-Abstract-head, Els-Abstract-text, Els-keywords) and the trailing sectPr
# (page setup/margins/columns). Delete everything else -- the "Here introduces
# the paper..." placeholder body, the Nomenclature/Structure/Tables/etc.
# instructions, the example table, and the trailing "Instructions to Authors"
# admin section.
paras = doc.paragraphs
anchor = paras[8]._p  # first paragraph to delete
...
# ---- fill in title / author / affiliation / abstract / keywords ----------
def set_para_text(paragraph, text):
    """Replace a paragraph's runs with a single plain-text run, keeping its
    paragraph style (some of these paragraphs originally held Word field
    codes for auto-populated title/author metadata -- plain text is more
    reliable for a one-shot export like this)."""
    for r in list(paragraph.runs):
        r._r.getparent().remove(r._r)
    paragraph.add_run(text)


set_para_text(paras[1], "Classifying Alzheimer's Disease from Real ADNI MRI Scans: "
                        "Finding and Fixing the Shortcuts a Model Can Cheat With")
set_para_text(paras[2], "Nikunj Bhalla")
set_para_text(paras[3], "[Add your institution / department here]")
paras[4].text = ""
set_para_text(paras[6],
...
set_para_text(paras[7],
    "Keywords: Alzheimer's disease; MRI classification; convolutional neural "
    "networks; ADNI; subject-level data leakage; cross-validation; shortcut learning")


# ---- helpers for the body, all appending (truncation above put the cursor
# right after the keywords line) --------------------------------------------
def h1(text):
    doc.add_paragraph(text, style="Els-1storder-head")


def h2(text):
    doc.add_paragraph(text, style="Els-2ndorder-head")


def p(text):
    doc.add_paragraph(text, style="Els-body-text")


def bullet(text):
    doc.add_paragraph(text, style="Els-bulletlist")


def caption(text, center=False):
    p = doc.add_paragraph(text, style="Els-caption")
    if center:
...
def reference(text):
    doc.add_paragraph(text, style="Els-reference")


def figure(path, cap, width=4.6):
    if not os.path.exists(path):
        doc.add_paragraph(f"[figure missing: {os.path.basename(path)}]")
        return
    pic_p = doc.add_paragraph()
    pic_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    pic_p.add_run().add_picture(path, width=Inches(width))
    # Template rule (its own instructions, verified against the example Fig. 1
    # caption in the template): captions go BELOW the figure, left-justified in
    # general, but "one-line captions of length less than column width... centered"
    # -- every caption here is a short one-liner well under the 6.5" column width,
    # so centered is what the template's own rule actually calls for.
    caption(cap, center=True)


def _set_cell_borders(cell, top=False, bottom=False):
    """Matches the template's own example table exactly (checked directly in its
    XML): 'Normal Table' style with NO vertical lines anywhere, a horizontal line
    above the header row, one below the header row, and one under the very last
    row -- nothing else. This is what the template's own instructions describe
    ('Only horizontal lines should be used within a table... immediately above
    and below the table'), reproduced from the template's own markup rather than
    guessed."""
...
def add_table(headers, rows, cap_text):
    ...
    for i, htext in enumerate(headers):
        ...
        cell.paragraphs[0].style = doc.styles["Els-table-text"]
    ...
    for row in rows:
        ...
        for i, val in enumerate(row):
            cells[i].paragraphs[0].style = doc.styles["Els-table-text"]
            cells[i].paragraphs[0].add_run(str(val))
            _set_cell_borders(cells[i], top=False, bottom=False)
    for cell in t.rows[-1].cells:
        _set_cell_borders(cell, top=False, bottom=True)
    doc.add_paragraph("")


# ----------------------------------------------------------- INTRODUCTION --
h1("1. Introduction")
p("Alzheimer's disease is the most common cause of dementia. Its course is a "
  "gradual loss of memory and independent function that worsens over years, and "
  "structural MRI is central to its clinical workup because atrophy in the "
  "hippocampus and surrounding medial temporal structures becomes visible well "
  "before end-stage disease. A classifier that reads this signal reliably could "
  "support earlier, more consistent screening than manual radiological review "
  "alone.")
p("The practical difficulty in this field is not training a convolutional network "
...
p("We treat that risk as a central methodological question rather than an "
...
p("Convolutional networks are the dominant tool for MRI-based Alzheimer's "
  ...
  "for grayscale MRI.")
p("A second, less frequently addressed strand of the literature concerns "
...
p("Site and scanner harmonization is a related, named problem in the wider "
...
p("This work sits at the intersection of the two strands above: standard CNN "
...
p("Data were obtained directly from the ADNI (Alzheimer's Disease Neuroimaging "
...
add_table(["Class", "ADNI1", "ADNI-GO/2", "Total"],
          [["Cognitively Normal (CN)", 127, 158, 285],
           ["Alzheimer's Disease (AD)", 108, 108, 216],
           ["Early MCI (EMCI)", 0, 227, 227],
           ["Late MCI (LMCI)", 0, 125, 125]],
          "Table 1. Subject counts by diagnostic class and ADNI acquisition phase.")
p("Each subject contributes one T1-weighted MPRAGE-family series. Subjects were "
  ...
  "one split.")
figure(os.path.join(FIG, "class_distribution.png"), "Fig. 1. Class distribution across the dataset.")

h2("3.2 Data Preprocessing")
p("Each subject's sagittally-acquired DICOM series is stacked into a 3D volume "
...
figure(os.path.join(FIG, "qc_v3_range.png"),
      "Fig. 2. Example extracted axial slice range across the anchored band, "
      "confirming consistent anatomical coverage.")

h2("3.3 Confound Detection Methodology")
p("Before any classification result is reported, each candidate confound is "
...
add_table(["#", "Candidate confound", "Test result", "Status"],
          ...
          "Table 2. Confound audit: what was tested, what was found, and what was fixed.")
p("A fifth variable, imaging site, was also measured and found to carry a "
...
p("Three architectures were compared, all consuming a single 224x224 axial slice "
  "per forward pass:")
...
p("ImageNet-pretrained initialization was tested for MobileNetV2 under three "
...
add_table(["Parameter", "Value"],
          ...
          "Table 3. Training configuration shared by all three architectures.")

h2("3.6 Evaluation Protocol")
p("Each subject's slice-level predictions are aggregated by averaging softmax "
...
p("Table 4 reports each architecture's performance on a single held-out test "
...
add_table(["Model", "Split", "n", "Accuracy", "Macro F1", "ROC AUC"],
          [["Custom CNN", "single split", 75, "73.3%", "0.725", "0.838"],
           ...
           ["MobileNetV2", "5-fold CV (headline)", 501, "74.1% [70.0, 77.7]", "--",
            "0.784 [0.743, 0.826]"]],
          "Table 4. AD-vs-CN performance, single split (illustrative) vs 5-fold "
          "cross-validation (headline).")
p("The cross-validated AUC interval excludes 0.5 by a wide margin, indicating a "
...
p("Per-model confusion matrices and training curves for all three architectures "
  "follow, each from the same 75-subject held-out split as Table 4, so the visual "
  "pattern of errors and the shape of the learning curve can be compared directly "
  "across architectures.")
figure(os.path.join(DEMO_FIG, "custom_cnn_confusion_matrix.png"), "Fig. 3a. Custom CNN confusion matrix, AD vs CN (n=75).")
figure(os.path.join(DEMO_FIG, "custom_cnn_training_curves.png"), "Fig. 3b. Custom CNN training curves, AD vs CN.")
figure(os.path.join(DEMO_FIG, "mobilenetv2_confusion_matrix.png"), "Fig. 3c. MobileNetV2 confusion matrix, AD vs CN (n=75).")
figure(os.path.join(DEMO_FIG, "mobilenetv2_training_curves.png"), "Fig. 3d. MobileNetV2 training curves, AD vs CN.")
figure(os.path.join(DEMO_FIG, "efficientnet_b0_confusion_matrix.png"), "Fig. 3e. EfficientNet-B0 confusion matrix, AD vs CN (n=75).")
figure(os.path.join(DEMO_FIG, "efficientnet_b0_training_curves.png"), "Fig. 3f. EfficientNet-B0 training curves, AD vs CN.")
p("All three show the same qualitative pattern: validation accuracy tracks "
...
figure(os.path.join(FIG, "gradcam_mobilenetv2_ADvsCN.png"),
      "Fig. 4. Grad-CAM attention overlays for the AD-vs-CN model, showing "
...
p("The harder four-stage task was also attempted, restricted to the ADNI-GO/2 "
...
add_table(["Model", "Subject accuracy", "Macro F1", "Soft-vote accuracy"],
          [["Custom CNN", "41.9%", "0.397", "40.9%"],
           ["MobileNetV2", "38.7%", "0.383", "36.6%"],
           ["EfficientNet-B0", "39.8%", "0.245", "37.6%"]],
          "Table 5. Four-stage single-split results (93 test subjects, "
          "majority baseline 36.7%).")
p("5-fold cross-validation over all 618 subjects gives a pooled estimate for the "
  ...
  "result.")
p("Confusion matrices and training curves for all three architectures on this "
...
figure(os.path.join(FIG, "fourway_confusion_custom_cnn.png"), "Fig. 5a. Custom CNN confusion matrix, four-stage task (n=93).")
figure(os.path.join(FIG, "fourway_curves_custom_cnn.png"), "Fig. 5b. Custom CNN training curves, four-stage task.")
figure(os.path.join(FIG, "fourway_confusion_mobilenetv2.png"), "Fig. 5c. MobileNetV2 confusion matrix, four-stage task (n=93).")
figure(os.path.join(FIG, "fourway_curves_mobilenetv2.png"), "Fig. 5d. MobileNetV2 training curves, four-stage task.")
figure(os.path.join(FIG, "fourway_confusion_efficientnet_b0.png"), "Fig. 5e. EfficientNet-B0 confusion matrix, four-stage task (n=93).")
figure(os.path.join(FIG, "fourway_curves_efficientnet_b0.png"), "Fig. 5f. EfficientNet-B0 training curves, four-stage task.")
p("EfficientNet-B0's confusion matrix is the clearest illustration: EMCI recall "
...
p("Zeroing skull, scalp, and background pixels (skull-stripping) around the "
...
h2("4.4 Discussion")
p("Two patterns recur across both tasks. First, model capacity does not "
...
# -------------------------------------------------------------- CONCLUSION -
h1("5. Conclusion")
p("We built an Alzheimer's MRI classification pipeline from raw ADNI DICOM data "
